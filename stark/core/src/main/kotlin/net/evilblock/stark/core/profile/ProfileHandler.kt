...
import net.evilblock.stark.core.profile.lookup.GeoLookup
...
abstract class ProfileHandler<T : Profile> {
    ...
    fun loadProfile(uuid: UUID, ipAddress: String?): T {
        ...
        if (ipAddress != null) {
            ...
            // async geo lookup
            if (profile.geoInfo == null) {
                ForkJoinPool.commonPool().execute {
                    val geoInfo = GeoLookup.lookup(ipAddress)

                    if (geoInfo != null) {
                        profile.geoInfo = geoInfo
                        collection.updateOne(Document("uuid", profile.uuid.toString()), Document("\$set", Document("geoInfo", geoInfo.toDocument())))
                    }
                }
            }
        }
    ...
    fun saveProfile(profile: T) {
        val grants = profile.rankGrants.stream().map<Document> { ProfileGrantSerializer.serialize(it) }.collect(Collectors.toList())
        val punishments = profile.punishments.stream().map<Document> { ProfilePunishmentSerializer.serialize(it) }.collect(Collectors.toList())
    ...
    fun findActivePunishment(uuid: UUID, ipAddress: String): Pair<UUID, ProfilePunishment>? {
        val orExpressions = Arrays.asList(
        ...
        for (profileDocument in collection.find(Document("\$or", orExpressions))) {
            val punishments = profileDocument.getList("punishments", Document::class.java)
                    .stream()
                    .map<ProfilePunishment> { ProfilePunishmentSerializer.deserialize(it) }
