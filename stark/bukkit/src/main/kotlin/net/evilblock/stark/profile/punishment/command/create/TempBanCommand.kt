...
object TempBanCommand {
    @Command(["tempban"], "stark.punishment.BAN.create", description = "Temporarily ban a player", async = true)
    @JvmStatic
    fun execute(sender: CommandSender, @Flag(value = ["s", "silentMode"], description = "Silently ban the player") silent: Boolean, @Param("target") target: BukkitProfile, @Param("time") time: String, @Param("reason", wildcard = true) reason: String) {
        ...
        try {
            ...
            if (sender is Player && !sender.hasPermission("stark.punishment.BAN.create.permanent")) {
