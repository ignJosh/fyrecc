...
object WhitelistCommands {
    ...
    fun mode(sender: CommandSender, @Param(name = "mode") mode: Int) {
        val modeType = when (mode) {
            0 -> WhitelistType.NONE
            1 -> WhitelistType.PURCHASED
            2 -> WhitelistType.MAINTENANCE
            else -> {
    ...
    fun whitelist(sender: CommandSender, @Param(name = "player") profile: BukkitProfile, @Param(name = "access") access: Int) {
        val modeType = when (access) {
            0 -> WhitelistType.NONE
            1 -> WhitelistType.PURCHASED
            2 -> WhitelistType.MAINTENANCE
            else -> {
    ...
    @JvmStatic
    @Command(names = ["whitelist read"], permission = "stark.whitelist", async = true)
    ...
    @JvmStatic
    @Command(names = ["wltokens use"], async = true)
    ...
    @JvmStatic
    @Command(names = ["wltokens give"], permission = "op", async = true)
