...
object UnmuteCommand {
    @Command(["unmute"], permission = "stark.punishment.MUTE.delete", description = "Unmute a player", async = true)
    @JvmStatic
    fun execute(sender: CommandSender, @Flag(value = ["s", "silentMode"], description = "Silently unmute the player") silent: Boolean, @Param("target") target: BukkitProfile, @Param("reason", wildcard = true) reason: String) {
        var issuer: UUID? = null
        if (sender is Player) {
            issuer = sender.uniqueId
        }

        val activePunishment = target.getActivePunishment(ProfilePunishmentType.MUTE)
        if (activePunishment == null) {
            sender.sendMessage("${target.getPlayerListName()} ${ChatColor.RED}is not muted.")
            return
        }
